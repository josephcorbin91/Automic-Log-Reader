...
public class MainMenu extends JFrame {
	...
	// Uneditable combobox for the software products
	...
	private void prepareGUI() throws ClassNotFoundException, InstantiationException, IllegalAccessException, UnsupportedLookAndFeelException{
		...
		btn_user.addActionListener(e -> {
			softwareSelected = softwareProductJComboBox.getSelectedItem().toString();
			user = new User(this, false,softwareSelected);
		...
		btn_admin.addActionListener(e -> {
			try {
				softwareSelected = softwareProductJComboBox.getSelectedItem().toString();
				loginFrame = new Login(this,softwareSelected);
			} catch (Exception e1) {
		...
		JPanel softwarSelectionPanel = new JPanel();
		contentPane.add(softwarSelectionPanel);
		String [] softwareProducts = { "One Automation", "Dollar Universe", "Applications Manager"};
		softwareProductJComboBox = new JComboBox<>(softwareProducts);
		softwarSelectionPanel.add(softwareProductJComboBox);
