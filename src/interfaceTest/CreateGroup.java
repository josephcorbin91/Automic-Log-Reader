...
public class CreateGroup extends JDialog{
	 ...
	 */
	public CreateGroup(AdminView admin, UserView view, String softwareSelected){
		prepareGUI(admin, view,softwareSelected);
	 ...
	 * as the administrator creates groups
	 */
	private void prepareGUI(AdminView admin, UserView view, String softwareSelected){
		try {
		...
		btnCreateGroup.addActionListener(e -> {
			if (Utility.noCheckBoxSelected(cbListKeyWords)){
				...
				try {
					saveGroups(admin, view, softwareSelected);
				} catch (Exception e1) {
					e1.printStackTrace();
				}
	 ...
	 */
	private void saveGroups(AdminView admin, UserView view, String softwareSelected) throws ClassNotFoundException, SQLException{
